...
Take the last width characters before the current one (i.e. s[i - width, i - 1], where s[i, j] means the substring of s from index i to index j, both inclusive), and call it the window.
...
In both of the above examples the resulting "compressed" string becomes even longer than the initial one. In fact, sliding window compression proves to be efficient for longer inputs. E.g.:
...
def losslessDataCompression(inputString, width):
    window = inputString[0]
    result = inputString[0]
    i = 1
    while i < len(inputString):
        window = inputString[0: i] if i < width else inputString[i-width: i]
        print inputString[i] + " ? " + window
        if window.find(inputString[i]) == -1:
        ...
        if len(window) < width:
            length = len(window)
        elif len(inputString) - i < width:
            length = len(inputString) - i
        else:
            length = width
        #length = width if len(inputString) - width > 0 else len(inputString) - width
        print '{}-{}-{}'.format(length, i, len(inputString)-i)
        for x in range(length, 0, -1):
            s_string = inputString[i:i+x]
            temp = window.find(s_string)
            print str(temp) + ' @@ ' + s_string + '++' + window
            if temp > -1:
                temp = i-width + temp if i > width else temp
